...
// Store para el estado de la UI del editor interno
export const useInternalEditorUIStore = create<EditorUIStore>()(subscribeWithSelector(immer((set, get) => ({
  // Estado inicial
  showLeftPanel: true,
  showRightPanel: true,
...
// Funciones de utilidad para trabajar con Polotno en el editor interno
export const internalEditorUtils = {
  // Guardar diseño
  saveDesign: () => {
    try {
  ...
  createPageWithDimensions: async (width: number, height: number, background = '#ffffff') => {
    console.log('=== INICIO createPageWithDimensions (Internal Editor) ===');
    ...
    try {
      ...
      if (page) {
        ...
        console.log('=== VERIFICACIÓN INMEDIATA (Internal Editor) ===');
        ...
        setTimeout(() => {
          console.log('=== FORZANDO ACTUALIZACIÓN (100ms) - Internal Editor ===');
        ...
        setTimeout(() => {
          console.log('=== SEGUNDO INTENTO (500ms) - Internal Editor ===');
      ...
      console.error('=== ERROR en createPageWithDimensions (Internal Editor) ===', error);
...
export const useInternalEditorStore = useInternalEditorUIStore;

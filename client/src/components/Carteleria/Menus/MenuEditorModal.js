...
import { GripVertical, ImagePlus, Plus, Trash2, UploadCloud, X } from 'lucide-react';
    ...
    <div ref={setNodeRef} style={style} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3">
        ...
        {block.block_type === 'product' ? (
          <div>
            <label className="label">Producto vinculado</label>
            <select
              className="input"
        ...
        {block.block_type === 'promotion' ? (
          <div className="space-y-3">
            <div>
              <label className="label">Promocion vinculada</label>
              <select
                className="input"
        ...
        {block.block_type === 'combo' ? (
          <div className="space-y-3">
            <div>
              <label className="label">Combo vinculado</label>
              <select
                className="input"
...
const MenuEditorModal = ({
  ...
  useEffect(() => {
    ...
    if (menu) {
      ...
      setBlocks(normalizeBlocks(menu.blocks || []));
    } else {
      setFormData({
        name: '',
        local_name: '',
  ...
  const handleAddBlock = (blockType) => {
    setBlocks((currentBlocks) =>
      normalizeBlocks([...currentBlocks, createBlock(blockType, lookupData)])
    );
  };
  ...
  const handleRemoveBlock = (blockId) => {
    setBlocks((currentBlocks) =>
      normalizeBlocks(currentBlocks.filter((block) => block.id !== blockId))
    );
  };
    ...
    <div className="modal-overlay">
      <div className="modal w-[min(96vw,1440px)] max-w-[1440px] h-[92vh] overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="flex h-[calc(92vh-72px)] flex-col overflow-hidden">
          <div className="grid grid-cols-1 gap-4 border-b border-gray-200 px-6 py-4 lg:grid-cols-4">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-500">
                {logoUploading
                  ...
                  : formData.logo_upload_id
                    ? 'Logo listo para guardarse en el menu.'
          ...
          <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[280px,1fr,420px]">
            <aside className="border-r border-gray-200 bg-gray-50 p-4">
              <h4 className="text-lg font-semibold text-gray-900">Bloques</h4>
              <p className="mt-1 text-sm text-gray-500">
                Agrega secciones vivas conectadas al catalogo del local.
              </p>
            ...
            <section className="overflow-y-auto p-4">
              <h4 className="text-lg font-semibold text-gray-900">Estructura del menu</h4>
              <p className="mt-1 text-sm text-gray-500">
                Reordena bloques y configura cada uno sin perder la cabecera fija.
              </p>
            ...
            <aside className="overflow-y-auto border-l border-gray-200 bg-gray-50 p-4">
              <h4 className="text-lg font-semibold text-gray-900">Vista previa</h4>
              <p className="mt-1 text-sm text-gray-500">
                Shell mobile con buscador y filtros siempre visibles.
              </p>

...
const Sidebar = ({ moduleConfig, onClose }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const BrandIcon = moduleConfig.brandIcon;
    ...
    <div className="flex h-screen flex-col bg-white shadow-xl">
      <div className="flex h-16 flex-shrink-0 items-center justify-between px-4 border-b border-gray-200">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <h1 className="text-lg font-semibold text-gray-900">{moduleConfig.appName}</h1>
      ...
      <nav className="flex-1 space-y-1 px-2 py-4">
        {moduleConfig.navigation.map((item) => {
